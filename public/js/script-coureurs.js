$( document ).ready(function() {
	...
	var colEdicHtml = '<tr><td class="idCOUR" hidden></td>' +
		...
		'<td class="equipeCOUR"><div style="display: none;"></div><input class="form-control input-sm"  value=""></td>' +
		'<td class="categorieCOUR"><div style="display: none;"></div><input class="form-control input-sm"  value=""></td>' +
	...
	rowAcep = function (but) {
		...
		var cont33 = $row.find('.equipeCOUR').find('input').val();
		$row.find('.equipeCOUR').html(cont33);
		var cont44 = $row.find('.categorieCOUR').find('input').val();
		$row.find('.categorieCOUR').html(cont44);
